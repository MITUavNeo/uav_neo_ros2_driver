...
successful build and deployment; the same recipe applies to racecar-neo (same
BCM2712 / Pi 5).
...
3. `coral-msi` overlay (`scripts/coral-msi.dts`): repoints `pcie@110000`
   ...
   vectors. This is the key fix; equivalent to `dtoverlay=pineboards-hat-ai` on
   Raspberry Pi OS, but board-agnostic.
...
```
drone setup coral        # or: bash scripts/setup_coral.sh
sudo reboot
```
...
Confirm the racecar DTB uses the same symbol: the overlay targets `&pcie1`, and
the fixup resolves it per-DTB, so no phandle editing is needed. Verify
`msi-parent` reads `pcie1`'s phandle after reboot.
