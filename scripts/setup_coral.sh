...
#   - 99-coral-edgetpu.rules udev rule. No reboot.
...
__CFG__
    ...
    echo "[4/5] Installing udev rule for Coral USB access..."
    ...
    if cmp -s "$UDEV_SRC" "$UDEV_DST" 2>/dev/null; then
        ...
        echo "  Udev rule installed. Covers pre-init (1a6e:089a) and post-init (18d1:9302) IDs."
    fi
fi
